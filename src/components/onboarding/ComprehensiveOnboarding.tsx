...
const ComprehensiveOnboarding: React.FC<ComprehensiveOnboardingProps> = ({ onComplete, onBack }) => {
...
function NutritionStep({ formData, updateFormData }: any) {
  const diets = [
    ...
    'Intermittent Fasting', 'Low Carb', 'Plant-Based', 'Other'
  ];
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            {diets.map((diet) => (
              <button
                key={diet}
                onClick={() => updateFormData({ dietPreference: diet })}
                className={cn(
...
function MentalWellnessStep({ formData, updateFormData }: any) {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
        Mental Wellness
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {[
            { id: 'low', name: 'Low', desc: 'Generally calm and relaxed' },
            { id: 'moderate', name: 'Moderate', desc: 'Some stress but manageable' },
            { id: 'high', name: 'High', desc: 'Often stressed or anxious' },
            { id: 'very-high', name: 'Very High', desc: 'Constantly overwhelmed' }
          ].map((level) => (
            <button
              key={level.id}
              onClick={() => updateFormData({ stressLevel: level.id })}
              className={cn(
                'p-4 text-left border-2 rounded-lg transition-all',
                formData.stressLevel === level.id
                  ? 'border-blue-500 bg-blue-50 dark:bg-blue-950/30'
                  : 'border-gray-200 hover:border-gray-300'
              )}
            >
...
function SupplementsStep({ formData, updateFormData }: any) {
  const commonSupplements = [
    'Multivitamin', 'Vitamin D', 'Omega-3', 'Probiotics', 'Magnesium',
    'Protein Powder', 'Creatine', 'B-Complex', 'Vitamin C', 'Zinc'
  ];

  const toggleSupplement = (supplement: string) => {
    const current = formData.currentSupplements || [];
    const updated = current.includes(supplement)
      ? current.filter((s: string) => s !== supplement)
      : [...current, supplement];
    updateFormData({ currentSupplements: updated });
  };

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
        Current Supplements & Medical Info
      </h2>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            {commonSupplements.map((supplement) => (
              <button
                ...
                className={cn(
                  'p-3 text-center border-2 rounded-lg transition-all text-sm',
                  (formData.currentSupplements || []).includes(supplement)
                    ? 'border-blue-500 bg-blue-50 dark:bg-blue-950/30'
                    : 'border-gray-200 hover:border-gray-300'
                )}
              >
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="List any medical conditions or medications (this helps us provide safer recommendations)..."
            rows={3}
          />
        </div>
      </div>
        ...
        <div className="p-4 border border-gray-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Help improve our recommendations (anonymous)
              </p>
            </div>
            <button
              ...
              className={cn(
                'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.dataSharing ? 'bg-blue-600' : 'bg-gray-200'
              )}
            >
              <span
                className={cn(
                  'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                  formData.dataSharing ? 'translate-x-5' : 'translate-x-0'
                )}
              />
            </button>
          </div>
