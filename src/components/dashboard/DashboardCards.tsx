  ...
  return (
    <div className="rounded-xl bg-white dark:bg-neutral-900 shadow-md border border-neutral-100 dark:border-neutral-800 p-7 w-full h-full">
      <div className="flex items-center gap-2.5 mb-5 text-left">
        ...
        <Info className="w-5 h-5 text-green-500 cursor-pointer" />
      </div>
      <DonutChart />
...
const DashboardCards: React.FC<DashboardCardsProps> = ({ 
  ...
  // Extract macro values (marked as used with underscore prefix)
  const { protein: _protein, carbs: _carbs, fat: _fat } = userProfile?.dailyMacros || { protein: 0, carbs: 0, fat: 0 };
  const { protein: _goalProtein, carbs: _goalCarbs, fat: _goalFat } = userProfile?.macroGoals || { protein: 0, carbs: 0, fat: 0 };
