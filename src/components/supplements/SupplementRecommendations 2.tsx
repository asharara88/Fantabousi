...
const SupplementRecommendations: React.FC<SupplementRecommendationsProps> = ({
  ...
  const handleAddToCart = async (supplement: Supplement) => {
    try {
      ...
      console.log(`${supplement.name} added to cart!`);
        ...
        <div className="flex items-center justify-between">
          <div className="text-right">
            <span className="text-xl font-bold text-gray-900 dark:text-white">
              {(supplement.price_aed || supplement.price || 0).toFixed(2)} AED
            </span>
          </div>
      ...
      <div className="p-6">
        ...
        <div className="space-y-2 mb-4">
          {stack.components.slice(0, 3).map((component) => (
            <div key={component.supplement_id} className="flex justify-between text-sm">
              <span className="text-gray-700 dark:text-gray-300">
                {component.name}
              </span>
              <span className="font-medium text-gray-900 dark:text-white">
                {component.price.toFixed(2)} AED
              </span>
            </div>
          ))}
